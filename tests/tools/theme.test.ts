...
describe('handleThemeCall — create_theme', () => {
  ...
  it('passes through extra properties (e.g. libraryId from multi-library dispatch)', async () => {
    // src/mcp/index.ts extracts libraryId to pick the correct CEM, then forwards
    // the original arg object into handleThemeCall. The schema must not reject
    // server-internal fields that legitimate multi-library callers include.
