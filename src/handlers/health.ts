...
async function parseHistoryFile(filePath: string): Promise<HistoryFileRaw> {
  const raw = await readFile(filePath, 'utf-8');
  const parsed: unknown = JSON.parse(raw);
  const result = HistoryFileSchema.safeParse(parsed);
  if (!result.success) {
    throw new Error(`Invalid health history file "${filePath}": ${result.error.message}`);
  }
  return {
...
async function readLatestHistoryFile(
  ...
  return parseHistoryFile(join(dir, files[0] as string));
}
...
export async function scoreComponent(
  ...
  throw new Error(`No health history for '${tagName}' and no CEM data provided for fallback`);
}
...
export async function getHealthTrend(
  ...
  if (selectedFiles.length === 0) {
    throw new Error(`No health history files found for '${tagName}'`);
  }
