...
export async function benchmarkLibraries(
  ...
  for (const lib of libraries) {
    if (lib.cemPath.includes('..')) {
      throw new MCPError('Path traversal (..) is not allowed', ErrorCategory.VALIDATION);
    }
    if (lib.cemPath.startsWith('/')) {
      throw new MCPError('Absolute paths are not allowed', ErrorCategory.VALIDATION);
    }
  }
  ...
  for (const lib of libraries) {
    const absPath = join(config.projectRoot, lib.cemPath);
