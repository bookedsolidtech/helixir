...
export async function handleLibraryCall(
  ...
  try {
    if (name === 'load_library') {
      ...
      if (packageName) {
        // Load from CDN using existing resolveCdnCem logic (register=true to store in CEM cache)
        const result = await resolveCdnCem(packageName, version, registry, config, true);
        // Also load under the user's chosen libraryId for explicit targeting.
